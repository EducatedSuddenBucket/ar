...
(async () => {  
  try {  
    ...
    // Generate or merge api/index.json with archive data.  
    ...
    // Merge new results into existingData  
    results.forEach(newRecord => {  
      ...
      if (existingDomain) {  
        newRecord.dates.forEach(newDate => {  
          // Only add the date if it doesn't exist already  
    ...
    // Generate index.html with a search bar and dynamic UI (lazy loads screenshots).  
    ...
    function renderArchive(data) {  
      ...
      data.domains.forEach(domainData => {  
        ...
        domainData.dates.forEach(dateEntry => {  
          ...
          // Set the screenshot URL in a data attribute instead of src  
          imgEl.setAttribute('data-src', dateEntry.screenshot);  
          // Initially, do not load the image  
          imgEl.style.display = 'none';  
          dateHeader.onclick = () => {  
            // On first click, assign the src if not set  
